erpnext.ProductSearch = class {
	...
	setupProductsContainer() {
		this.products_container = this.search_dropdown.append(`
			<div id="product-results mt-2">
				<div id="product-scroll" style=" overflow-x: hidden; max-height: 300px">
				</div>
			</div>
		`).find("#product-scroll");
	}
		...
		recents.forEach((key) => {
			html += `
				<div class="recent-search mr-1" style="font-size: 13px">
					...
					<span class="darkblue-text">${ key }</span>
				</div>
			`;
		});
	...
	populateResults(product_results) {
		...
		product_results.forEach((res) => {
			let thumbnail = res.wensite_image || res.thumbnail || '/assets/erpnext/images/ui-states/cart-empty-state.png';
			html += `
				<div class="dropdown-item" style="display: flex;">
					<img class="item-thumb col-2" src=${thumbnail} />
					<div class="col-9" style="white-space: normal;">
						<h6 class="darkblue-text"><a href="/${res.route}">${res.web_item_name}</a></h6><br>
