	...
	*/
	...
	prepare_toolbar() {
		this.products_section.append(`
			<div class="toolbar d-flex my-3 mx-1">
			</div>
		`);
		this.prepare_search();
		this.prepare_view_toggler();

		frappe.require('/assets/js/zekasa-product-listening.min.js', function() {
	...
	add_paging_section(settings) {
		...
		if($("body").attr("data-path") == 'all-products'){
			if (this.products) {
				let paging_html = `
				...
				if(this.product_count > page_length){
				paging_html += `
					<button  data-start="${ start - page_length }"
						style="float: left" ${prev_disable} class="mr-4">
						${ __("Prev") }
					</button>`;
				paging_html += `
					<button class="btn btn-default btn-next ml-3" data-start="${ start + page_length }"
						${next_disable}>
						${ __("Next") }
					</button>
				`;
		...
		$(".toolbar").append(`
			<div class="input-group col-8 p-0">
				<div class="dropdown w-100" id="dropdownMenuSearch">
					...
					<div class="search-icon search_icon_for_products">
	...
	render_view_toggler() {
		$(".toolbar").append(`<div class="toggle-container d-flex col-4 p-0 justify-content-end"></div>`);
			...
			$(".toggle-container").append(`
				<div class="form-group mb-0" id="toggle-view">
					<button id="${ icon }" class="btn ${ view }  mr-2">
						<span>
							<svg class="icon icon-md ${view}-product">
